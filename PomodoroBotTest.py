...
async def create_break_job(pomo_data):
    ...
    break_job = scheduler.add_job(reschedule_job, "date", 
                run_date = datetime.utcnow()+timedelta(seconds=pomo_data['break_time']), args=[pomo_data])
...
def create_secondary_task(pomo_data):
    if pomo_data['breaks_left'] > 0:
        ...
        secondary_run_time = datetime.utcnow() + timedelta(seconds=pomo_data['pomo_time'])
